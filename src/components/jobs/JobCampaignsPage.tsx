...
export default function JobCampaigns() {
  ...
  async function loadStateAndJobs(selectedRecruiter = recruiter) {
    try {
      ...
      const r1 = await fetch(`${API}/api/publish/state`, {
      ...
      setPub((prev) => {
        const next: PublishState = prev
          ? {
              ...
              byChannel: JSON.parse(JSON.stringify(prev.byChannel || {})),
            }
          : { channels: [], byChannel: {} };
        next.byChannel[channelId] ||= {};
        if (active)
          next.byChannel[channelId][String(jobId)] = {
  ...
  const filtered = useMemo(() => {
    ...
    if (recruiter !== "__ALL__") {
      arr = arr.filter(
        (j) => recruiterOf(j).toLowerCase() === recruiter.toLowerCase()
      );
    }
    arr.sort(
      (a, b) =>
        +new Date(createdOf(b) || 0) - +new Date(createdOf(a) || 0)
    );
    return arr;
  }, [jobs, recruiter]);
    ...
    <div style={{ padding: 16 }}>
      ...
      >
        ...
        >
          {[
            "__ALL__",
            ...Array.from(new Set(jobs.map(recruiterOf).filter(Boolean))).sort((a, b) =>
              a.localeCompare(b)
            ),
          ].map((r) => (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead style={{ background: "#f3f4f6" }}>
                <tr>
                  ...
                  {pub?.channels?.map((c) => (
                    <th key={c.id} style={{ textAlign: "center", padding: 10 }}>
                      {c.id}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {page.length === 0 ? (
                  <tr>
                    <td
                      colSpan={(pub?.channels?.length || 0) + 3}
                  ...
                  page.map((j) => (
                    <tr key={j.id} style={{ borderTop: "1px solid #e5e7eb" }}>
                      <td style={{ padding: 10 }}>
                        <div>
                          <strong>{j.title || j.jobTitle || "(untitled)"}</strong>
                        </div>
                        <div style={{ color: "#6b7280", fontSize: 12 }}>
                          Created: {fmtDate(createdOf(j)) || "—"}
                        </div>
                        <div
                          style={{
                            color: "#6b7280",
                      ...
                      {pub?.channels?.map((c) => (
                        <td key={c.id} style={{ textAlign: "center", padding: 10 }}>
                          <Switch
                            ...
                            title={
                              !isAreaValid(j)
                                ? "Select a valid area (county/unitary) first."
                                : `Publish to ${c.id}`
                            }
                          />
                        </td>
