...
// --- PATCH: recruiter handling fix ---
function normaliseJob(j) {
  const id = j.id || j.jobId;
  const area = j.areaName || j.location || "";
  const override = (id && LOCATION_OVERRIDES[id]) || {};

  // Handle recruiter object (e.g. { id, displayName })
  let recruiterObj = null;
  let recruiterName = "Unknown recruiter";

  if (j.recruiter && typeof j.recruiter === "object") {
    recruiterObj = j.recruiter;
    recruiterName = j.recruiter.displayName || j.recruiter.name || "Unknown recruiter";
  } else if (typeof j.recruiterName === "string" && j.recruiterName.trim()) {
    recruiterName = j.recruiterName.trim();
  } else if (typeof j.recruiter === "string" && j.recruiter.trim()) {
    recruiterName = j.recruiter.trim();
  } else if (typeof j.postedBy === "string" && j.postedBy.trim()) {
    recruiterName = j.postedBy.trim();
  }

  return {
    ...j,
    id,
    recruiter: recruiterObj || j.recruiter || { displayName: recruiterName },
    recruiterName,
    title:
      (typeof j.title === "string" && j.title) ||
      (typeof j.jobTitle === "string" && j.jobTitle) ||
      "Untitled job",
